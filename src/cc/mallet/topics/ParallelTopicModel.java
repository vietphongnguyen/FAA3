...
import phong.FAA_GUI.FAA3_GUI;
import phong.FAA_GUI.generateTopics;
import phong.FAA_GUI.processAutoDetectNumberOfTopics;
...
public class ParallelTopicModel implements Serializable {
	...
	public void estimate () throws IOException {
		...
		for (int iteration = 1; iteration <= numIterations; iteration++) {
			...
			// Caculate progress bar in TopicModeling//////////////////////////////////////////////////////////////////////////////////////////////
			long currentTime,traveredTime, remainingTime;
			double percent;
			if (FAA3_GUI.process_GenerateTopics_running) {
				FAA3_GUI.progressBarMallet.setValue(generateTopics.count);
				percent = 100 * generateTopics.count / generateTopics.numIterations;

				currentTime = System.nanoTime(); // get current time
				traveredTime = (currentTime - generateTopics.startTime);
				...
				FAA3_GUI.progressBarMallet.setString(
						"Doing " + (generateTopics.count) + " / " + generateTopics.numIterations + " ( " + percent
								+ " % )  Time remaining: " + remainingTime / 60 + " m " + remainingTime % 60 + " s");
			}
			if (FAA3_GUI.ProcessAutoDetectNumberOfTopics_running) {
				FAA3_GUI.progressBarMallet.setValue(processAutoDetectNumberOfTopics.count);
				percent = 100 * processAutoDetectNumberOfTopics.count / processAutoDetectNumberOfTopics.numIterations;

				currentTime = System.nanoTime(); // get current time
				traveredTime = (currentTime - processAutoDetectNumberOfTopics.startTime);

				remainingTime = (long) (traveredTime * ((100 - percent) / percent));
				remainingTime = remainingTime / 1000000000; // converted from nanoseconds to second
				processAutoDetectNumberOfTopics.count++;
				FAA3_GUI.progressBarMallet.setString(
						"Doing " + (processAutoDetectNumberOfTopics.count) + " / " + processAutoDetectNumberOfTopics.numIterations + " ( " + percent
								+ " % )  Time remaining: " + remainingTime / 60 + " m " + remainingTime % 60 + " s");
			}
			////////////////////////// Check if the process of generating topics had been canceled by user  /////////////////////////////////////
			if (FAA3_GUI.process_GenerateTopics_running && FAA3_GUI.process_GenerateTopics.isCancelled()) { 
				FAA3_GUI.Out("The process of generating topics had been canceled by user  \n"); 
			...
			if (FAA3_GUI.ProcessAutoDetectNumberOfTopics_running && FAA3_GUI.threadProcessAutoDetectNumberOfTopics.isCancelled()) { 
				FAA3_GUI.Out("The process of 'Auto Detect Number Of Topics' had been canceled by user  \n"); 
				FAA3_GUI.btnGetCompositionTable.setEnabled(true); 
				FAA3_GUI.btnEstimateTopics.setEnabled(true);
				FAA3_GUI.btnXCancel.setEnabled(false);
